...
plt = plot(viz, select=1:length(labels), ask=FALSE)
# print(summary(plt$plots[[1]]$ggObj))
# ggsave("plots/test.png", plt$plots[[1]]$ggObj)2
...
test_experiment = "thresholds"
experiment = load_experiment(test_experiment)
```

```{r treatment-race}
plot_experiment = function(experiment, name) {
  plt = ggplot(experiment, aes(x=race, y=sampling_benefit_per_child)) +
    ylab("Race-weighted misallocation per eligible child") +
  ...
  print(plt)
  ggsave(sprintf("plots/misalloc_%s.png", name), dpi=300, width=5, height=6)
}
...
for (experiment_name in c(
  ...
  plot_experiment(experiment, experiment_name)
}
```
