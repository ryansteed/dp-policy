...
comparison = grouped %>%
  mutate(two_or_more_races_race_pct = 100-rowSums(across(ends_with("race_pct")))) %>%
  mutate(two_or_more_races_race_pct = if_else(two_or_more_races_race_pct < 0, 0, two_or_more_races_race_pct)) %>%
  pivot_longer(
    ends_with("race_pct"),
    values_to="race_pct",
    names_to="race"
