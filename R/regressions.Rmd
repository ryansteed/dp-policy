...
raw = fread("results/discrimination_laplace_eps=0.1.csv")
```
...
ggplot(sorted, aes(x=race, y=sampling_benefit_per_child)) +
  plot +
  ylab("Race-weighted misallocation per eligible child") +
  geom_crossbar(
    data=sorted, 
...
ggsave(sprintf("plots/misalloc_%s.png", kind), dpi=300, width=5, height=6)
...
```{r gam-mr}
sampling_only = F
gam_mr = run_regs(df %>% filter(trial < 2), sampling_only)
...
```{r test}
# saveRDS(gam_dp, sprintf("results/regressions/%s_sampling=%s.rds", "baseline", F))
# gam_dp = readRDS(sprintf("results/regressions/%s_sampling=%s.rds", "baseline", F))
gam_mr = get_gam("baseline", sampling_only, F)
```
...
```{r treatments}
test_experiment = "moving_average"
experiment = load_experiment(test_experiment)
plot_experiment(experiment, test_experiment)
```
...
for (experiment_name in c(
  ...
  plot_race(experiment, experiment_name)
  ...
  for (t in unique(experiment$treatment)) {
    gam_mr = get_gam(
      ...
      F,
      F,
      experiment %>% filter(treatment == t, trial < 10)
    )
    plot_gam(gam_mr, sprintf("%s_%s", experiment_name, t))
  }
}
```
