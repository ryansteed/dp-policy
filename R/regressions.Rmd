...
```{r}
comparison = race_comparison_long(grouped, "race_aggregate")
```

```{r}
res = bootstrap_benefit(comparison %>% filter(race == "White", treatment=="Hold harmless"))
res
```
```{r}
# comparison %>%
...
comparison %>%
  ...
  distinct(treatment, race, dp_benefit_per_child_eligible_std_error, dp_benefit_per_child_eligible_bias, dp_benefit_per_child_eligible)
```
...
for (experiment_name in c(
  ...
  plot_race(experiment, experiment_name, 2)
