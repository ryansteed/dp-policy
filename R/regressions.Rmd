...
```{r}
plot(experiment$black_or_african_american_race_pct, log(experiment$true_pop_total))
plot(experiment$white_race_pct, log(experiment$true_pop_total))
mean(experiment$true_pop_total[experiment$black_or_african_american_race_pct > 0.5])
mean(experiment$true_pop_total[experiment$white_race_pct > 0.5])
```
...
cplt = cowplot::plot_grid(
  plotlist=map2(
    list(
      ...
      gam(
        misalloc ~ s(log(average_household_size_of_renter_occupied_unit_housing_tenure_est), bs="tp"),
        method="REML",
        data=df_reg
      )
    ),
